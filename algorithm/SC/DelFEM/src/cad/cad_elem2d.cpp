	...
	{	// Mesh
		...
		for (unsigned int idiv = 0; idiv < ndiv; idiv++)
		{
			...
			if (idiv == 0)
				...
				poi0 = po_s + h0*relcomsh[(idiv - 1) * 2 + 0] + v0*relcomsh[(idiv - 1) * 2 + 1];
			Com::CVector2D poi1;
			if (idiv == ndiv - 1)
				poi1 = po_e;
			else
				poi1 = po_s + h0*relcomsh[idiv * 2 + 0] + v0*relcomsh[idiv * 2 + 1];
			for (unsigned int jdiv = idiv + 2; jdiv < ndiv; jdiv++)
			{
				...
				if (jdiv == 0)
					...
					poj0 = po_s + h0*relcomsh[(jdiv - 1) * 2 + 0] + v0*relcomsh[(jdiv - 1) * 2 + 1];
				Com::CVector2D poj1;
				if (jdiv == ndiv - 1)
					poj1 = po_e;
				else
					poj1 = po_s + h0*relcomsh[jdiv * 2 + 0] + v0*relcomsh[jdiv * 2 + 1];
				if (IsCross_LineSeg_LineSeg(poi0, poi1, poj0, poj1) != 0)
					return true;
			}
		}
		return false;
...
bool Cad::CEdge2D::IsCrossEdge_ShareBothPoints(const CEdge2D& e1, bool is_share_s0s1) const
{
	...
	{	// intersection between line and arc
		return false;
	}
	else if (this->itype == 1 && e1.itype == 0)
	{
