...
bool CDesigner2D_Cloth::Cad_Remove(Cad::CAD_ELEM_TYPE itype, unsigned int id)
{
	...
	if (itype == Cad::EDGE)
	{
		...
		if (!cad_2d.RemoveElement(itype, id)) return false;
		if (is_l_l_cut != is_l_r_cut)
		{	// àÍï˚Ç™ÉÅÉbÉVÉÖÇ™Ç ÇËÅCàÍï˚Ç…ÉÅÉbÉVÉÖÇ™Ç»Ç¢èÍçáÇÕÅCÇ≈Ç´ÇÈÇæÇØÉÅÉbÉVÉÖÇçÏÇÈ
...
void CDesigner2D_Cloth::Draw(unsigned int idraw_object)
{
	...
	if (idraw_object == 0)
	{ // analysis and cad_rigid_loop, cad_edge and cad_vertex
		if (pAnalysis != 0)
		{
...
bool CDesigner2D_Cloth::Cad_AddCutLine(const Com::CVector2D& pos, const Com::CVector2D& poe)
{
	...
	////
	if (id_ls > 0 && id_le > 0 && id_ls == id_le)
	{ // diamond dart
	...
	std::map<unsigned int, int>& loopId2SvgIdMap, int coarse_ntri, int detail_ntri)
{
	...
	this->setIdVCad_NeedFollow.clear();
	this->pAnalysis = pAnalysis;
	pAnalysis->SetModelClothFromSvg(cad_2d, mesh_2d, svgManager, slider_deform, 
		aSymIdVPair, loopId2SvgIdMap, coarse_ntri, detail_ntri);
	this->InitDrawer();
}
