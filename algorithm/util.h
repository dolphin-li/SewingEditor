...
namespace ldp
{
	...
	inline ldp::QuaternionF calcQuaternion(float xTheta, float yTheta, float zTheta)
	{
		return ldp::QuaternionF().fromAngleAxis(zTheta, ldp::Float3(0, 0, 1))
			* ldp::QuaternionF().fromAngleAxis(yTheta, ldp::Float3(0, 1, 0))
...
#undef max
	...
	inline ldp::Mat2f vec2Tensor(ldp::Float2 v)
	{
		if (v.sqrLength() != 0)
			v.normalizeLocal();
		ldp::Mat2f M;
	...
	inline ldp::Mat2f vec2Tensor_noNormalize(ldp::Float2 v)
	{
		ldp::Mat2f M;
		M(0, 0) = v[0] * v[0];
		M(1, 0) = v[1] * v[0];
	...
	inline ldp::Float2 tensor2Vec(ldp::Mat2f ten)
	{
		Eigen::Matrix2f A;
		A(0, 0) = ten(0, 0);
		A(0, 1) = ten(0, 1);
		...
		Eigen::Matrix2f E = solver.eigenvectors();
		ldp::Float2 dir(E(0, 1), E(1, 1));
		if (dir[0] < 0)
			dir = 0.f - dir;
		return dir;
	}
